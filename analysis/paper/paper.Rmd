...
```
# NOTES To Madeleine
- see edits in survey_duration.R to aid interpretation by the viewer (e.g., changes in x axis tick locations, etc.)
- documentation in scripts
- **NEXT Priority Milestones**: note - all references to line numbers are prior to any other edits!
- finish graphs of first & last observation per year for each target species (around line 386);**ultimately** add a fitted linear trend line to each of those (we need to account for frequency of surveying, so some subtle effort things here, but can resolve that later);
- graphs of species occurrence (around line 407) for each of target species; 
...
```{r SiteAccess, eval=TRUE, cache = FALSE, include = FALSE}
# STILL NEED TO CREATE
# source("../scripts/survey_access.R", echo=FALSE)
```

```{r Fig-SiteAccess, echo=FALSE, eval=TRUE, cache=TRUE, fig.show= 'asis', fig.cap="",warning= FALSE}
# print(plot_access)
```
...
```{r Fig-SurveyEffort, echo=FALSE, eval=TRUE, cache=TRUE, fig.show= 'asis', fig.cap="",warning= FALSE}
print(plot_duration)
```
...
```{r Fig-AllSppFreq, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis', fig.cap="blah blah about consistency in target species occurrence. Maybe something about new spp showing up?"}
...
```{r Fig-SiteSurveyCompletion, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis', fig.cap="Need to add caption.",warning=FALSE}
...
```{r Fig-SurveyTiming1, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis', fig.cap="The annual dates of the first and last survey have both varied by over a month across the years."}

print(plot_alldates)
```


JOEL - I specified ncol = 3 in the facet command but my html doesn't update with it (I know the command itself works since I can see the zoom in the script)? Are you able to see it? and/or does it work in solving the space problem?
```{r Fig-SurveyTiming2, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis', fig.cap=""}
...
# scatterplot of species' first and sightings compared to survey season start
...
```{r Fig-SurveyDates2, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis', fig.cap="The last sighting of each species each year.", warning=FALSE}
print(plot_timing_2)
```
...
```{r Fig-SpeciesTiming2, echo=FALSE, eval=TRUE, cache=TRUE, fig.show= 'asis', fig.cap="For each of the target species, the dates on which they were observed in any of the sample areas."}
