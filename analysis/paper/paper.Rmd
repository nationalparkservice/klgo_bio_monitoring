...
```
# NOTES To Madeleine
- see edits in survey_duration.R to aid interpretation by the viewer (e.g., changes in x axis tick locations, etc.) $\checkmark$
- documentation in scripts $\checkmark$
- **NEXT Priority Milestones**: note - all references to line numbers are prior to any other edits!
- finish graphs of first & last observation per year for each target species (around line 386);**ultimately** add a fitted linear trend line to each of those (we need to account for frequency of surveying, so some subtle effort things here, but can resolve that later); $\checkmark$
- graphs of species occurrence (around line 407) for each of target species; $\checkmark$
...
- **Joel & Madeleine** will discuss survey_dates.R commands and how to modify them so that we get a sense of when surveys were & when target species was first (or last) seen each year. E.g., separate out any filtering process stemming from survey effort levels and timing. (thinking some thing like showing open circles for survey events PRIOR to first sighting...)
- Need to think about way to winnow out target species that do not occurr w/ adequate frequency to report on. E.g., those only seen <3 or 5 years??? Look at phenology graphs and determine threshold to apply. Consider some species N-ward and S-ward migration routes may differ.
- Joel will finish script uploading AOU data csv object & using to order Target Species
...
```{r Target_Species, eval=TRUE, echo=FALSE, warning = TRUE}
# selects out only target species listed above
#
source("../scripts/target_species.R")
```
...
> SECOND graph: [THIS IS SECONDARY PRIORITY] x axis is Time of Day, perhaps broken in to 1 or 2 hour blocks , y axis is ....this is a tough one so totally open to your ideas. How about y axis is % [0, 1.0] and for each year there is a different curve showing the % of planned surveys (as defined above) that were successfully completed that season within that time block; facet by site. So we are seeing whether there are consistently problematic times of day at each site. 
> JOEL: some updates on this! for now, times are in hour blocks. There can't be lines/curves for year since they are unreadable (up/down). I also tried to use color as a factor but then the graphs are crowded (so many years). So I have 2 alternatives, I don't really like either but some are more readable than others. See below.

```{r SiteAccess, eval=TRUE, cache = FALSE}

source("../scripts/survey_access.R", echo=FALSE)
```

```{r Fig-SiteAccess1, echo=FALSE, eval=TRUE, cache=TRUE, fig.show= 'asis',fig.height=20, fig.width=12, fig.cap="",warning= FALSE}

print(plot_success)
```

```{r Fig-SiteAccess2, echo=FALSE, eval=TRUE, cache=TRUE, fig.show= 'asis',fig.height=15, fig.width=12, fig.cap="",warning= FALSE}
...
```{r Fig-SurveyEffort, echo=FALSE, eval=TRUE, cache=TRUE, fig.show= 'asis', fig.height=12, fig.width=12, fig.cap="",warning= FALSE}
...
> JOEL: I did some graphs for species occurrence that fit the description but not sure if they were what you were looking for? They do show species occurrence relative to others; as annual/daily percentage of daily observations in which spp was sighted (percentage calculated by annual/daily obs per species over all total obs - is this what you meant? let me know).
Lines didn't work though (lots of ups and downs)
...
```{r Fig-AllSppFreq1, echo=FALSE, eval=TRUE, warning=FALSE, cache=FALSE, fig.show= 'asis', fig.height=20, fig.width=12, fig.cap="blah blah about consistency in target species occurrence. Maybe something about new spp showing up?"}

print(plot_occurrenceDay)
```
...
*Survey Completion*
> Ultimately, re-order these sections to flow better. Completion then Duration, etc., etc.
...
```{r Fig-SiteSurveyCompletion, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis',fig.height=15, fig.width=18, fig.cap="Need to add caption.",warning=FALSE}
...
```{r Fig-SurveyTiming1, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis',fig.height=8, fig.width=12, fig.cap="The annual dates of the first and last survey have both varied by over a month across the years."}
...
```{r Fig-SurveyDates1, echo=FALSE, eval=TRUE, cache=FALSE, fig.show= 'asis', fig.cap="The first sighting of each species each year.",fig.height=20, fig.width=12, warning=FALSE}
